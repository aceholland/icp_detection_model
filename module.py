...
def generator_attention(input_shape=512,
    ...
    def _downsample(ip, filter_size, kernel_size, norm, stride_size=2):
        ip = layers.Conv1D(filters=filter_size, kernel_size=kernel_size,
                           strides=stride_size, padding='same', use_bias=False)(ip)
        if norm != 'none':
            ip = layers.normalization(norm)(ip)
        ip = layers.Activation(ip, activation='leaky_relu')
        return ip

    def _upsample(ip, filter_size, kernel_size, norm, stride_size=2,
                  drop_rate=0.5, apply_dropout=False):
        ip = layers.DeConv1D(filters=filter_size, kernel_size=kernel_size,
                             strides=stride_size, padding='same', use_bias=False)(ip)
        if norm != 'none':
            ip = layers.normalization(norm)(ip)
        if apply_dropout:
            ip = layers.Dropout(rate=drop_rate)(ip)
        ip = layers.Activation(ip, activation='relu')
        return ip

    h = inputs = keras.Input(shape=(input_shape,))
    h = keras.layers.Lambda(lambda x: tf.expand_dims(x, axis=1))(h)
    h = keras.layers.Lambda(lambda x: tf.expand_dims(x, axis=3))(h)
    ...
    h = keras.layers.Lambda(lambda x: tf.squeeze(x, axis=1))(h)
    h = keras.layers.Lambda(lambda x: tf.squeeze(x, axis=2))(h)
